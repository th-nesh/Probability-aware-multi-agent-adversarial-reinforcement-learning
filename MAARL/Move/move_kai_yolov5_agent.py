...
def convert_pix_to_grid(x_r,y_r):
    "applies the roation of -69° to the robo cords to get the crods in the grind we measure on"
    x_g = x_r*np.cos(1.204)-y_r*np.sin(1.204)
    y_g = x_r*np.sin(1.204)+y_r*np.cos(1.204)
    return x_g,y_g
...
def modify_map(map_ref, obstacles_org, obstacles):
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        # visu_adv_traj_map_draw_prm = visu_adv_traj_map_draw
        list_shifts_table = []
        list_shifts_box = []
        obstacles_org = deepcopy(obstacles)
        # while not rospy.is_shutdown():
        detected_objects = []
        i = 0 
        while len(detected_objects)<2 or i<5:
            i = i+1
            ...
            visu_adv_traj_map = copy.deepcopy(map_ref)
            obstacles = deepcopy(obstacles_org)        

            # visu_adv_traj_map_draw = deepcopy(visu_adv_traj_map_draw_prm)
            global real_data

            shift_pixels = []
            detected_objects = []
            # view D=1 R=2 U=3 L=4 
            view = 1
            table_corner = [0,1,3,2][view]
            box_corner = [0,1,2,3][view]
            ...
            for obj,label in zip(cord,labels):
                ...
                if label == 2:
                    ...
                    # 1 is the box
                    obstacle_index = 2
                    shift_pix = (right_corner[0]-obstacles[-obstacle_index].corners[table_corner][0],right_corner[1]-obstacles[-obstacle_index].corners[table_corner][1])
                    shift_pix = convert_pix_to_grid(*shift_pix)
                    if view in [1,3]:
                        shift_pix = (shift_pix[0],0)
                        list_shifts_table.append(shift_pix[0])
                    else:
                        shift_pix = (0,shift_pix[1])
                        list_shifts_table.append(shift_pix[1])
                    shift_pix = convert_grid_to_pix(*shift_pix)
                    detected_objects.append(obstacles[-obstacle_index])
                    shift_pixels.append(shift_pix)
                    with open('dist_csvs/error_dist_table_4.csv','a') as f1:
                        write = csv.writer(f1)
                        write.writerow([list_shifts_table[-1]])
                ...
                # here we process the box
                if label == 3:
                    list_of_depth.append(infer_depth(obj[1]*600,obj[3]*600,0.54))
                    list_of_depth.append((infer_width(obj[0]*800,list_of_depth[0]),infer_width(obj[2]*800,list_of_depth[0])))
                    ...
                    # 1 is the box
                    obstacle_index = 1
                    shift_pix = (right_corner[0]-obstacles[-obstacle_index].corners[box_corner][0],right_corner[1]-obstacles[-obstacle_index].corners[box_corner][1])
                    detected_objects.append(obstacles[-obstacle_index])
                    shift_pix = convert_pix_to_grid(*shift_pix)
                    if view in [1,3]:
                        shift_pix = (shift_pix[0],0)
                        list_shifts_box.append(shift_pix[0])
                    else:
                        shift_pix = (0,shift_pix[1])
                        list_shifts_box.append(shift_pix[1])
                    shift_pix = convert_grid_to_pix(*shift_pix)
                    ...
                    # print(curr_loc)
                    with open('dist_csvs/error_dist_box_4.csv','a') as f:
                        write = csv.writer(f)
                        write.writerow([list_shifts_box[-1]])

                shifted_obst = obstacle_adversary(detected_objects,shift_pixels)
                #shifted_obst = detected_objects
                """this is if you just want the outlines visible on the original map"""
                # visu_adv_traj_map = visu_adv_traj_map.convert('RGB')
                # visu_adv_traj_map_draw = ImageDraw.Draw(visu_adv_traj_map)
                # for i in range(0, len(shifted_obst)):
                #     for j in range(0, len(shifted_obst[i].corners)):
                #         visu_adv_traj_map_draw.point([(shifted_obst[i].corners[j])], fill=(255, 0, 0))

                #     for j in range(0, len(shifted_obst[i].borders)):
                #         visu_adv_traj_map_draw.line([shifted_obst[i].borders[j][0], shifted_obst[i].borders[j][1]], fill=(0, 255, 0))
                visu_adv_traj_map = modify_map(visu_adv_traj_map, obstacles_org, shifted_obst)
            ...
            # cv2.waitKey(1)
        print('objects detection was off by: ', shift_pixels)

        trajectory_vanilla, nodes_vanilla, edges_all_vanilla = initialize_traj(visu_adv_traj_map,shifted_obst, nodes=None)
        traj_updt = []



        for i in range(len(trajectory_vanilla)):
            traj =  get_amcl_from_pixel_location(trajectory_vanilla[i].x,trajectory_vanilla[i].y,*base_inf)
            traj_updt.append(traj)

        for trajectory_node, counter in zip(traj_updt, range(len(traj_updt))):

            # visu_adv_traj_map = copy.deepcopy(visu_adv_traj_map)
            if counter <len(traj_updt)-1:
                ...
                # visu_adv_traj_map_draw.line([(self.trajectory_adv[counter].coordinates[0], self.trajectory_adv[i].coordinates[1]), (self.trajectory_adv[i+1].coordinates[0], self.trajectory_adv[i+1].coordinates[1])], fill=(0, 0, 255))
                # visu_adv_traj_map_draw.point([(self.trajectory_adv[counter].coordinates[0], self.trajectory_adv[i].coordinates[1])], fill=(0, 0, 255))
                # visu_adv_traj_map_draw.point([(self.trajectory_adv[counter+1].coordinates[0], self.trajectory_adv[i+1].coordinates[1])], fill=(0, 0, 255))
            cv2.imshow('Camera', out)
            cv2.imshow('map', np.asarray(visu_adv_traj_map))
            cv2.waitKey(1)
