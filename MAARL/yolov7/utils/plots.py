...
def plot_3d_box(corners_3D, img, p, color=None, label=None, line_thickness=3):
    ...
    # print(corners_3D)
    c, jacobian = cv2.projectPoints(corners_3D,rvec,tvec, p, None)
    # front face
    c = c[:,0,:]
    c = c.astype(int)
    # print('2d points: ',c)
    c = tuple(map(tuple, c))
    # print('2d points: ',c[0])
    # print('2d points: ',type(c))
    cv2.line(img, c[0], c[1], color, thickness=tl, lineType=cv2.LINE_AA)
    ...
    cv2.line(img, c[6], c[7], color, thickness=tl, lineType=cv2.LINE_AA)
    cv2.line(img, c[7], c[4], color, thickness=tl, lineType=cv2.LINE_AA)
    # sides
    cv2.line(img, c[1], c[5], color, thickness=tl, lineType=cv2.LINE_AA)
    cv2.line(img, c[2], c[6], color, thickness=tl, lineType=cv2.LINE_AA)
    cv2.line(img, c[4], c[0], color, thickness=tl, lineType=cv2.LINE_AA)
    cv2.line(img, c[3], c[7], color, thickness=tl, lineType=cv2.LINE_AA)
    # if label:
    #     tf = max(tl - 1, 1)  # font thickness
    #     t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
    #     c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
